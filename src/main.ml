...
let run command =
  ...
  main_catch_all
  @@ fun () ->
  let eval () = Term.eval ~catch:false ~argv:Sys.argv command in
  match eval () with
...
type nix_dep =
  ...
  ; filtered_constraints:
      (OpamTypes.filter list * (relop * string) OpamFormula.formula) list }
...
type nix_expr =
  [ `NTrue
  | `NFalse
  | `NAnd of nix_expr * nix_expr
  | `NOr of nix_expr * nix_expr
  | `NNot of nix_expr
  | `NImpl of nix_expr * nix_expr
  | `NVar of string
  | `NNull
  | `NAp of nix_expr * nix_expr
  | `NAttr of nix_expr * string
  | `NEq of nix_expr * nix_expr
  | `NNeq of nix_expr * nix_expr
  | `NList of nix_expr list
  | `NStr of string
  | `NStrI of [`NLit of string | `NInterp of nix_expr] list
  | `NSet of (string * nix_expr) list
  | `NIf of nix_expr * nix_expr * nix_expr
  | `NPath of string ]
...
let nix_if c t e =
  match c with `NTrue -> t | `NFalse -> e | _ -> `NIf (c, t, e)
...
let nix_stringify x =
  nix_if (nix_is_bool x) (nix_if x (nix_str "true") (nix_str "false")) x

let nix_stri ls =
  match ls with [] -> nix_str "" | [`NLit l] -> nix_str l | _ -> `NStrI ls

let rec nix_strc ls r =
  match (ls, r) with
  | ls, `NStr "" -> ls
...
let nix_str_append l r =
  match (nix_stringify l, nix_stringify r) with
...
let nix_escape s =
  Re.replace
    (Re.compile (Re.alt [Re.char '\\'; Re.char '"'; Re.str "${"; Re.str "\n"]))
    ~f:(fun g ->
      "\\" ^ (fun c -> if c = "\n" then "n" else c) (Re.Group.get g 0) )
    s

let shell_escape s =
  "'" ^ Re.replace (Re.compile (Re.char '\'')) ~f:(fun _ -> "'\\''") s ^ "'"

let argname_of_pkgname p =
  match OpamPackage.Name.to_string p with "ocamlfind" -> "findlib" | n -> n

let nix_ver_of_pkg pkg =
  `NAp
    ( `NAttr (`NAttr (`NVar "stdenv", "lib"), "getVersion")
    , `NVar (argname_of_pkgname pkg) )

let nix_ver_of_filter flt = `NStr flt

let rec resolve_ident = function
  | (_ :: _ :: _ as ps), v ->
      List.fold_left nix_and nix_true
      @@ List.map (fun p -> resolve_ident ([p], v)) ps
  | [], "name" -> nix_var "pname"
  | [], "jobs" -> nix_str "1"
  | [], "make" -> nix_str "make"
  ...
  | ["ocaml"], "native-dynlink" ->
      nix_not (`NAttr (nix_var "stdenv", "isMips"))
  | ["ocaml"], "preinstalled" -> nix_true
  | [p], "installed" -> nix_neq (nix_var p) (nix_var "null")
  | ps, v ->
      raise
      @@ Wat
           ( OpamFilter.to_string
           @@ FIdent
                ( List.map
                    (function
                      | "_" -> None | p -> Some (OpamPackage.Name.of_string p))
                    ps
                , OpamVariable.of_string v
                , None ) )

let rec nix_bool_of_filter flt =
  match flt with
  | FBool true -> nix_true
  ...
  | FIdent (ps, v, None) ->
      resolve_ident
        ( List.map
            (OpamStd.Option.map_default OpamPackage.Name.to_string "_")
            ps
...
let nix_bool_of_formula nix_bool_of_atom =
  let rec go nix_bool_of_empty formula =
    match formula with
...
let rec pp_nix_expr_prec prec ppf nb =
  ...
  | `NAnd (l, r) ->
      let paren =
        match prec with `PElse | `PImpl | `POr | `PAnd -> false | _ -> true
      in
  ...
  | `NOr (l, r) ->
      let paren =
        match prec with `PElse | `PImpl | `POr -> false | _ -> true
      in
      if paren then pp_print_text ppf "(" else () ;
      ...
      if paren then pp_print_text ppf ")" else ()
  | `NImpl (l, r) ->
      let paren = match prec with `PElse -> false | _ -> true in
      if paren then pp_print_text ppf "(" else () ;
  ...
  | `NAttr (l, a) ->
      let paren =
        match prec with
        | `PElse | `PImpl | `POr | `PAnd | `PEq | `PNot | `PAp | `PAttr ->
            false
        | _ -> true
      in
      if paren then pp_print_text ppf "(" else () ;
      ...
      pp_print_text ppf a ;
      if paren then pp_print_text ppf ")" else ()
  | `NStr s -> fprintf ppf "@[<h>\"%s\"@]" @@ nix_escape s
  ...
  | `NIf (cond, tbranch, ebranch) ->
      let paren = match prec with _ -> false in
      if paren then pp_print_text ppf "(" else () ;
      fprintf ppf "@[if@ @[" ;
      ...
      fprintf ppf "@,@]@,@]" ;
      if paren then pp_print_text ppf ")" else ()
  | `NPath p -> fprintf ppf "%s" p

and pp_nix_str ppf pieces =
  pieces
  |> List.iter (fun piece ->
         let open Format in
         match piece with
...
let pp_nix_pkg ?opam ppf nix_pkg =
  ...
  | Some file ->
      ...
      fprintf ppf "@]*/@;" ) ;
  pp_nix_args ppf
    ( [nix_arg "stdenv" None; nix_arg "opam" None; nix_arg "fetchurl" None]
    @ List.map arg_of_dep (OpamPackage.Name.Map.bindings nix_pkg.deps) ) ;
  nix_pkg.deps
  |> OpamPackage.Name.Map.iter (fun name {is_required; filtered_constraints} ->
         filtered_constraints
         |> List.iter (fun (filters, constraints) ->
                let guard =
                  List.fold_left
                    (fun l r -> nix_and l @@ nix_bool_of_filter r)
                    ( if is_required then nix_true
                    else nix_neq (nix_var @@ argname_of_pkgname name) nix_null
                    )
                    filters
                in
                let cond =
                  nix_bool_of_formula
                    (nix_bool_of_constraint name)
                    `NTrue constraints
                in
  ...
  nix_pkg.conflicts
  |> OpamPackage.Name.Map.iter (fun name {filtered_constraints; _} ->
  ...
  | x :: xs ->
      ...
      pp_nix_expr ppf
      @@ List.fold_left
           (fun x y -> nix_str_append x @@ nix_str_append (nix_str "\n") y)
           x xs ) ;
  fprintf ppf ";@ " ;
  ...
  @@ nix_list
       (List.map
          (fun p -> nix_var @@ argname_of_pkgname p)
          (OpamPackage.Name.Map.keys nix_pkg.deps)) ;
  fprintf ppf ";@ " ;
  ...
  @@ nix_list
       (List.map
          (fun p -> nix_var @@ argname_of_pkgname p)
          (OpamPackage.Name.Map.keys nix_pkg.prop_deps)) ;
  fprintf ppf ";@ " ;
  fprintf ppf "configurePhase = \"true\"" ;
  fprintf ppf ";@ " ;
  fprintf ppf
    "patches = (x: stdenv.lib.concatStringsSep \" \" (stdenv.lib.concatLists \
     x))@ " ;
  pp_nix_expr ppf nix_pkg.patches ;
  fprintf ppf ";@ " ;
  fprintf ppf
    "buildPhase = stdenv.lib.concatMapStringsSep \"\\n\" (x: \
     stdenv.lib.concatStringsSep \" \" (stdenv.lib.concatLists x))@ " ;
  ...
  fprintf ppf ";@ " ;
  fprintf ppf
    "installPhase = \"runHook preInstall; mkdir -p $out; for i in *.install; \
     do ${opam.installer}/bin/opam-installer -i --prefix=$out \
     --libdir=$OCAMLFIND_DESTDIR \\\"$i\\\"; done\"" ;
  fprintf ppf ";@ " ;
...
let nixdep_of_filtered_constraints fc =
  let rec go fc =
    ...
    | Or (l, r) -> (
        let lp, la = go l in
        let rp, ra = go r in
        match (lp, rp) with [], [] -> ([], Or (la, ra)) | _, _ -> raise Waat )
...
let parse_ident s =
  ...
  | Some (p, last) ->
    ...
    | None -> (OpamStd.String.split p '+', last)
    | Some (_p, _) -> raise @@ Wat s

let nix_expand_string s =
  let f = function
    | `Delim g ->
        ...
        if str = "%%" then nix_str "%"
          ...
          resolve_ident
          @@ parse_ident (String.sub str 2 (String.length str - 4))
    | `Text t -> nix_str (shell_escape t)
  in
  List.fold_right
    (fun x -> nix_str_append (f x))
    (Re.split_full string_interp_regex s)
    (nix_str "")
...
let nix_expr_of_commands cmds =
  `NList
    ( cmds
    |> OpamStd.List.filter_map (fun (args, flt) ->
           nix_optionals_opt
             (OpamStd.Option.map_default nix_bool_of_filter `NTrue flt)
             (nix_expr_of_args args) ) )

let rec nixdeps_of_depends depends =
  match depends with
  ...
  | And (l, r) ->
      OpamPackage.Name.Map.union
        (fun x y ->
          { x with
            filtered_constraints=
              x.filtered_constraints @ y.filtered_constraints } )
        (nixdeps_of_depends l) (nixdeps_of_depends r)
  | Or (l, r) ->
      raise
      @@ Wat
           ( OpamFilter.string_of_filtered_formula l
           ^ " OROROR "
           ^ OpamFilter.string_of_filtered_formula r )
  | Atom (name, cs) ->
      OpamPackage.Name.Map.singleton name @@ nixdep_of_filtered_constraints cs

let rec nixdeps_of_depopts depopts =
  match depopts with
  | Empty -> OpamPackage.Name.Map.empty
  | Block _ -> raise Waat
  | And (l, r) ->
  ...
  | Or (l, r) ->
      OpamPackage.Name.Map.union
        (fun x _ -> x)
        (nixdeps_of_depopts l) (nixdeps_of_depopts r)
  | Atom (name, cs) ->
      OpamPackage.Name.Map.singleton name
      @@ {(nixdep_of_filtered_constraints cs) with is_required= false}

let nixdeps_of_depexts depexts =
  depexts
  |> List.map (fun (pkgs, flt) ->
         nix_optionals_opt (nix_bool_of_filter flt)
           (nix_list @@ List.map nix_str pkgs)
         |> function
         | None -> []
         | Some (`NList xs) ->
             xs
             |> List.map (function `NStr name ->
...
let nix_src_of_opam name version opam =
  OpamGlobalState.with_ `Lock_write
  @@ fun gt ->
  ...
  let fetch_source_job =
    ...
    | None -> Done (Ok None)
    | Some u ->
        OpamRepository.pull_file_to_cache (OpamPackage.to_string nv) ~cache_dir
          ~cache_urls (OpamFile.URL.checksum u)
        ...
        | _ ->
            Ok
              (Some
                 ( OpamFile.URL.url u
                 , None
                 , OpamRepository.cache_file cache_dir
                     (List.hd (OpamFile.URL.checksum u)) ))
  in
    ...
    |> List.map (fun dl -> function
         ...
         | Ok rs ->
             ...
             | Ok (Some (url, name, file)) ->
                 OpamSystem.make_command "nix-hash"
                   [ "--type"
                   ; "sha256"
                   ; "--base32"
                   ; "--flat"
                 ...
                 if OpamProcess.is_success result then
                   let fetch =
                     `NAp
                       ( nix_var "fetchurl"
                       , `NSet
                           [ ("url", `NStr (OpamUrl.to_string url))
                           ; ("sha256", `NStr (List.hd @@ result.r_stdout)) ]
                       )
                   in
                   match name with
                   ...
                   | Some name ->
                       Done
                         (Ok (fun (src, xs) -> rs @@ (src, (name, fetch) :: xs)))
                 else Done (Error "SHA256 computation failed") )
  in
  let blankSrc = `NStr "/var/empty" in
  let r =
    OpamProcess.Job.run @@ OpamProcess.Job.seq jobs (Ok (fun xs -> xs))
  in
  match r with Error e -> raise @@ Wat e | Ok srcs -> srcs (blankSrc, [])

let union_of_nixdeps =
  OpamPackage.Name.Map.union (fun d1 d2 ->
...
let nix_of_opam ?name ?version opam =
  let name = match name with Some x -> x | None -> OpamFile.OPAM.name opam in
  let version =
    match version with Some x -> x | None -> OpamFile.OPAM.version opam
  in
  let deps =
    nixdeps_of_depends
      (And
         ...
         , Atom
             ...
             , Atom
                 (Filter (FIdent ([], OpamVariable.of_string "build", None)))
             ) ))
  in
  ...
  let extra_src =
    extra_src
    @ List.map
        (fun (b, _) -> (b, nix_path @@ OpamFilename.Base.to_string b))
        extra_files
  in
  (* TODO handle descr *)
...
let nixify =
  ...
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "Given an $(i,opam) file,  outputs recommendations, warnings or \
         errors on stderr." ]
  in
  let files =
    let open Arg in
    value
    & pos_all OpamArg.existing_filename_dirname_or_dash []
    & info ~docv:"FILES" []
        ~doc:
  ...
  let nixify global_options files package warnings_sel =
    ...
    let files =
      ...
      | files, None ->
          ...
          |> List.flatten
      | [], Some pkg -> (
          OpamGlobalState.with_ `Lock_none
          @@ fun gt ->
        ...
        (fun err opam_f ->
          try
            ...
            let enabled =
              ...
              in
              fun w ->
                try OpamStd.IntMap.find w map with Not_found -> default
            in
            ...
            |> OpamStd.Option.iter (fun upgraded ->
                   Format.printf "%a" (pp_nix_pkg ~opam:upgraded)
                   @@ nix_of_opam upgraded ) ;
            err || failed
          with
...
let () =
  OpamStd.Sys.at_exit (fun () -> flush stderr ; flush stdout) ;
  run nixify
